...
SKIP: {
	...
	skip('Test::Exception not installed', 13) if ($@);
	
	# test check_prototype
	lives_ok { List::Vectorize::check_prototype(1, '$') };
	lives_ok { List::Vectorize::check_prototype(1,1,1,1, '$+') };
	lives_ok { List::Vectorize::check_prototype(1,1,1,1, '${1,5}') };
	lives_ok { List::Vectorize::check_prototype([1,2], '\@') };
	lives_ok { List::Vectorize::check_prototype({}, '\%') };
	lives_ok { List::Vectorize::check_prototype(\1, '\$') };
	lives_ok { List::Vectorize::check_prototype(\\1, '\$') };
	lives_ok { List::Vectorize::check_prototype(sub{1}, '\&') };
	lives_ok { List::Vectorize::check_prototype(\*STDIN, '\*') };
	lives_ok { List::Vectorize::check_prototype(*STDIN, '*') };
	lives_ok { List::Vectorize::check_prototype(1, {}, '$\%') };
	lives_ok { List::Vectorize::check_prototype([], {}, '($|\@)\%') };
	dies_ok { List::Vectorize::check_prototype([], '$') } 'prototype unmatch';
}
